...
var underscore, jquery, backbone, backbonewreqr, backbonebabysitter, backbonemarionette, _base_, _collection_, composite, _item_, layout, marionettecomponents;
(function () {
  // Baseline setup
  // --------------
  ...
  Marionette.Region = Marionette.Object.extend({
    ...
    show: function (view, options) {
      ...
      if (_shouldDestroyView) {
        this.empty();  // A `destroy` event is attached to the clean up manually removed views.
 ...
 * Copyright (c) 2014
 */
_collection_ = function (Marionette, Base) {
  /* -----------------------------------------------------------------------------
   * CollectionComponent
   * ---------------------------------------------------------------------------*/
  return Base.extend({ View: Marionette.CollectionView });
}(backbonemarionette, _base_);
/*!
 * composite.js
 * 
 * Copyright (c) 2014
 */
composite = function (Marionette, Base) {
  /* -----------------------------------------------------------------------------
   * CompositeComponent
   * ---------------------------------------------------------------------------*/
  return Base.extend({ View: Marionette.CompositeView });
}(backbonemarionette, _base_);
/*!
 * item.js
 * 
 * Copyright (c) 2014
 */
_item_ = function (Marionette, Base) {
  /* -----------------------------------------------------------------------------
   * ItemComponent
   * ---------------------------------------------------------------------------*/
  return Base.extend({ View: Marionette.ItemView });
}(backbonemarionette, _base_);
/*!
 * layout.js
 * 
...
layout = function (Marionette, Item) {
  ...
  return Item.extend({
    ...
    show: function (regionName, componentName, Constructor, options, preventDestroy) {
      ...
      this.view[regionName].show(child.instance.view, { preventDestroy: preventDestroy });
    ...
    showingIn: function (regionName) {
      var showing = this.showing[regionName];
      return showing ? showing.instance : null;
    },
    /**
...
}(_base_, _collection_, composite, _item_, layout);
